...
Use agent-skills:test-case-design before TDD when the selected slice has non-trivial or uncertain coverage.
...
3. Validate an existing READY Test Planner, invoke agent-skills:test-case-design when coverage is non-trivial or uncertain, or use the TDD mini planner for one obvious behavior or confirmed defect
4. Select the highest-priority READY case and write its failing test (RED)
